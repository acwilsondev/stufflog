...
class TestStufflogApp(unittest.TestCase):
    ...
    def test_load_stufflog_with_git_pull(self):
        ...
        self.mock_file_service.get_stufflog_path.assert_called_once_with(self.test_category)
    ...
    def test_load_stufflog_nonexistent(self):
        ...
        self.assertIn(f"No stufflog found for category '{self.test_category}'", str(context.exception))
    ...
    def test_save_stufflog_with_git_push(self):
        ...
        # Verify interactions
        self.mock_file_service.save_stufflog.assert_called_once_with(self.test_category, self.test_data)
    ...
    def test_init_stufflog_existing(self):
        ...
        # Verify the error message
        self.assertIn(f"Stufflog for category '{self.test_category}' already exists", str(context.exception))
    ...
    def test_add_entry_duplicate(self):
        ...
        # Verify the error message
        self.assertIn(f"Entry titled '{existing_title}' already exists", str(context.exception))
    ...
    def test_delete_entry_nonexistent(self):
        ...
        # Verify the error message
        self.assertIn(f"No entry titled '{nonexistent_title}' found", str(context.exception))
    ...
    def test_query_entries(self):
        ...
        results = self.app.query_entries(self.test_category, greater_than=3)
        ...
        results = self.app.query_entries(self.test_category, less_than=5)
        ...
        results = self.app.query_entries(self.test_category, after="2023-01-01T12:00:00")
        self.assertEqual(len(results), 2)  # Should include Entry 2 and Entry 3
        titles = [entry["Title"] for entry in results]
        self.assertIn("Entry 2", titles)
        self.assertIn("Entry 3", titles)
        ...
        results = self.app.query_entries(self.test_category, before="2023-01-03T12:00:00")
    ...
    def test_integration_with_real_services(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Test querying
            entries = app.query_entries("test", greater_than=4)
